...
## Decisiones de seguridad (ajustadas con tu criterio)

1. **Alcance de la policy IAM — decidido: `SecurityAudit` + `ViewOnlyAccess` por defecto.** El wizard pide ambas managed policies de AWS como estándar, porque el producto audita seguridad *y* costes *y* arquitectura, y `SecurityAudit` sola no cubre varios `Describe`/`List` de inventario (EC2/RDS/Cost Explorer). Opción secundaria en el wizard: "solo seguridad" (`SecurityAudit`) para el cliente cauto, marcando en la UI qué partes del informe quedarán incompletas. Así no hay que pedirle al cliente que reautorice al activar la fase de costes.
...
3. **Identidad auditora — decidido: rol fijo `Clodaro-Auditor` ahora, no después.** El trust policy de cada cliente apuntará al ARN de un rol/principal de servicio dedicado en mi cuenta (`arn:aws:iam::<mi-account>:role/Clodaro-Auditor`), no a mis claves personales. Mis claves solo se usan para asumir `Clodaro-Auditor`, y desde ahí se hace el AssumeRole al rol del cliente (cadena de dos saltos). Así puedo rotar mis claves sin tocar la trust policy de ningún cliente. Implica: campo de configuración para el ARN del auditor, un paso de verificación en `manage-clients` que compruebe que puedo asumirlo, y documentación de la trust policy que necesita `Clodaro-Auditor` (confiar en mi principal IAM). Se implementa en el bloque 2, antes de conectar ningún cliente.
